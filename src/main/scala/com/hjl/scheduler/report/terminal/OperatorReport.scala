...
/**
  ...
  */
class OperatorReport extends JobComputing {
  ...
  override def process(): Unit = {
    ...
    resultDF.repartition(1).write.mode(SaveMode.Append).json(ReportConstant.OPERATOR_SINK_JSON_PATH)
    resultDF.write.mode(SaveMode.Append).jdbc(load.getString("jdbc.url"), ReportConstant.OPERATOR_SINK_TABLE_NAME, prop)
