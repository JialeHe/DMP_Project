...
object BaiduLBSHandler {
  ...
  def parseBusinessTagBy(lng: String, lat: String) = {
    ...
    if (statusCode == 200) { // HTTP.OK
      ...
      if (status == 0) {
        ...
        formatted_address = resultObject.get("formatted_address").getAsString.replaceAll(",", ";")
    ...
    formatted_address.concat("|").concat(business)
  }
  ...
  def requetParams(lng: String, lat: String) = {
    ...
    /**
      * http://api.map.baidu.com/geocoder/v2/?callback=renderReverse&location=31.05383%2C120.7212&output=json&pois=1&ak=ABiIl3LnSMkCiFO7YbFsIahAZLVwvvC8&sn=ad39156a8fcd311fbd00548f03e23632
      *
      * callback=renderReverse&location=31.05383%2C120.7212&output=json&pois=1&ak=ABiIl3LnSMkCiFO7YbFsIahAZLVwvvC8&sn=ad39156a8fcd311fbd00548f03e23632
      */
    ...
    paramsMap.put("ak", TagConstant.AK)

    // 请求的参数
    val paramsStr = toQueryString(paramsMap)

    // 生成SN
    val wholeStr = new String("/geocoder/v2/?" + paramsStr + TagConstant.SK)
